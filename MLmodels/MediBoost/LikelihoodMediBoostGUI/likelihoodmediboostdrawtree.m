function likelihoodmediboostdrawtree(tree,dataset)
% LIKELIHOODMEDIBOOSTDRAWTREE(tree) Returns the diagram of a decision tree object 
% created under the MediBoost paradigm. For more information see:
%
...
%
% SEE ALSO
%   likelihoodmediboostchoosefeat, likelihoodmediboostprunetree, likelihoodmediboosttrain, likelihoodmediboostvalue
    ...
    width = getwidth(tree)*100 + 120;
    depth = getdepth(tree)*100 + 120;
    ...
    likelihoodmediboostdrawnode(tree,width/2,depth-80)
    ...
    [cnames,rnames,data,columnwidth] = likelihoodmediboosttable(dataset);
    
    % Showing the uitable
    t = uitable(figure1,'Data',data,'ColumnWidth',{columnwidth},'ColumnName',cnames,'RowName',rnames,'Position',[0,405,100,100]);
    t.Position(3) = t.Extent(3);
    t.Position(4) = t.Extent(4);

    % recursive function to draw the lines of the tree
    function likelihoodmediboostdrawnode(tree,x,y)
        if tree.terminal==0
            % Get the width of each branch
            width1 = getwidth(tree.left)*100;
            width2 = getwidth(tree.right)*100;
            ...
            likelihoodmediboostdrawnode(tree.left,left + width1/2,y - 100);
            likelihoodmediboostdrawnode(tree.right,right - width2/2,y - 100);
        else
            txt = sprintf('\n\n%d',sign(tree.value));
            text(x - 20,y,txt);
        end
    ...
    function width = getwidth(tree)
        ...
        if isempty(tree.right) && isempty(tree.left)
            ...
            width = getwidth(tree.right) + getwidth(tree.left);
        end
    end
    ...
    function depth = getdepth(tree)
        ...
        if isempty(tree.right) && isempty(tree.left)
            ...
            depth = max(getdepth(tree.right),getdepth(tree.left)) + 1;
        end
    end
end